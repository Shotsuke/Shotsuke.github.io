...
BGP是一种自治系统（AS）之间使用的路径矢量路由协议，主要用于互联网的路由选择，被称为互联网的“中枢”协议。它为数据包找到最佳路径，从而实现全球范围内网络的互联。

在BGP中，分组并不是路由到一个特定的目的地址，而是路由到CIDR化的前缀，其中每个前缀表示一个子网或一个子网的集合。作为一种AS间的路由选择协议，BGP为每台路由器提供了一种完成以下任务的手段：
